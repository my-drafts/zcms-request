...
const uploadClean = function(zrequest){
	const R = zt.al(zrequest, { o: zrequest, else: {} });
	const r = 'UPLOAD';
	return function(){
		const items = Object.keys(R[r]).map(function(field){
			return R[r][field].map(function(file){
				return file.path;
			});
		});
		const paths = [].concat(items).map(function(path){
